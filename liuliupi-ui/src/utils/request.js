...
axios.interceptors.request.use(function (config) {
  // 在发送请求之前做些什么
  console.log(`[Axios Request] Proxying request to: ${config.url}`);
  return config;
}, function (error) {
  // 对请求错误做些什么
  console.error(`[Axios Error] Proxy request failed, error: ${error.message}, time: ${new Date().toLocaleString()}`);
...
axios.interceptors.response.use(function (response) {
  if (response.data !== null && response.data.hasOwnProperty("code") && response.data.code !== 200) {
    console.log(`[Axios Request] Proxying response to: ${response.data}`);
  ...
  // 对响应错误做点什么
  console.error(`[Axios Error] Proxy response failed, error: ${error.message}, time: ${new Date().toLocaleString()}`);
  return Promise.reject(error);
});
